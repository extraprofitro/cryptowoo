...
class CW_AddressList {
	...
	static function get_address_from_list( $currency, $order_id ) {
		// Delete the address list from cache to avoid concurrency issues when creating addresses during checkout.
		$index_key = self::get_address_list_key( $currency );
		wp_cache_delete( $index_key, 'options' );
	...
	static function save_address_list( $currency, $addresses ) {
		// Do not autoload so that we can do wp_cache_delete to avoid concurrency issues when creating addresses.
		return update_option( self::get_address_list_key( $currency ), $addresses, false );
	}
	 ...
	 * @return string
	 */
	public static function get_delete_list_button( $currency ): string {
		...
		return sprintf(
			'<div class="%2$s" id="delete_address_list_%1$s">Delete %1$s Address List</div>',
			$currency,
			$button_class
		);
	}
