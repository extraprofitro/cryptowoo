...
class CW_Order_Processing extends CW_Singleton_Array {
	...
	/**
	 * Decline checkout order for acquire lock timeout before creating address. Adds customer notice and logs the error.
	 */
	public function decline_order_address_acquire_lock_failed() {
		$order_id = $this->get_id();

		// Translators: %1$d: order number.
		$format = __( 'Order #%1$d: Failed to acquire lock before creating a payment address' );
		$msg    = sprintf( $format, $order_id );
		CW_AdminMain::cryptowoo_log_data( 0, __FUNCTION__, $msg, 'emergency' );
	...
	public function add_customer_notice_address_error() {
		$this->add_customer_error_notice( 'An error occurred while creating the payment address. Please try again in a few seconds.' );
	}
	...
	public function add_customer_notice_amount_error() {
		$this->add_customer_error_notice( 'An error occurred while calculating the digital currency amount. Please try again in a few seconds.' );
	}
