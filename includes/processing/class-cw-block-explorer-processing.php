...
class CW_Block_Explorer_Processing {
			...
			do {
				...
				if ( false !== strpos( $status, 'success' ) ) {
					...
					if ( ! isset( $fallback_explorers ) ) {
                        ...
                        // TODO: Log an error if no fallback explorers exist.
                    ...
                    // TODO: Log that the api response failed and that we are attempting fallback with another api.
				}
			} while ( false === strpos( $status, 'success' ) );
		}

        // TODO: Different error logging and/or email or without email for when fallback setting is disabled.
		if ( false === strpos( $status, 'success' ) ) {
	...
	public static function process_unpaid_orders( $payment_details ) {
		...
		foreach ( $payment_details as $info ) {
			...
			$amount_difference        = self::calculate_amount_difference( $info );
			$underpayment_upper_limit = (float) cw_get_option( 'underpayment_notice_range' )[2];
			$is_fully_paid            = ( $info->get_is_paid() || $amount_difference >= 0 );
			$is_paid_underpay_limit   = ! $amount_unconfirmed && $percentage_paid >= $underpayment_upper_limit;
			$is_paid                  = $is_fully_paid || $is_paid_underpay_limit;
	...
	private static function process_unpaid_order( $info ) {
		...
		$address_in_use = self::calculate_received_confirmed_plus_received_unconfirmed( $info ) > 0 ? true : false;
		...
		if ( 1 === $info->get_timeout() ) {
			...
			if ( $info->get_received_unconfirmed() > 0 || $info->get_received_confirmed() > 0 ) {
				...
				if ( empty( $tx_confirmed ) ) {
					...
					$full_amount_pending = self::calculate_received_confirmed_plus_received_unconfirmed( $info );
	 ...
	 * @return int
	 */
	private static function calculate_received_confirmed_plus_received_unconfirmed( $payment_details ) {
	...
	private static function calculate_percentage_paid( $payment_details ) {
		return ( $payment_details->get_received_confirmed() / $payment_details->get_crypto_amount_due() ) * 100;
	...
	private static function calculate_amount_difference( $payment_details ) {
		return $payment_details->get_received_confirmed() - $payment_details->get_crypto_amount_due();
	}
...
*/
	...
	public static function processing_api_error_action( $error ) {
        ...
        $last_sent_seconds_ago = time() - (int) $last_sent;

		// Max 1 email every day.
		if ( ! $last_sent || $last_sent_seconds_ago > DAY_IN_SECONDS ) {
			// Send email to admin.
			$to       = get_option( 'admin_email' );
			$blogname = get_bloginfo( 'name', 'raw' );
