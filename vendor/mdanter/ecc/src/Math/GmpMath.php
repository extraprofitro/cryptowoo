...
class GmpMath implements GmpMathInterface
{
    ...
    {
	    // There is a PHP bug in version 8.3.14, 8.2.26 and 8.4.1 that is causing a fatal PHP error.
	    // TODO: It is unknown in what version it will be fixed so we will update the version when we know.
	    if (
		    version_compare( PHP_VERSION, '8.2.26', '>=' )
		    && version_compare( PHP_VERSION, '8.3.0', '<' )
		    ||
		    version_compare( PHP_VERSION, '8.3.14', '>=' )
		    && version_compare( PHP_VERSION, '8.4.0', '<' )
		    ||
		    version_compare( PHP_VERSION, '8.4.0', '>=' )
	    ) {
		    // A solution is to use the PHP exponentiation operator instead of gmp_pow() for the affected version.
		    // This solution is discussed at https://github.com/php/php-src/issues/16870#issuecomment-2499293925.
		    return $base ** $exponent;
	    }

	    // Known unaffected PHP versions will continue using the gmp_pow function for now.
