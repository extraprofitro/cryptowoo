...
class CW_Admin_Notice {
	...
	/**
	 * Store admin notices as they get printed to avoid printing any admin notice more than once.
	 *
	 * @var string[]
	 */
	public static array $admin_notices_printed = array();
	...
	public function print_notice() {
		...
		// Store the admin notice id as printed, so we can check if it is already printed next time this method runs.
		self::$admin_notices_printed[ $this->id ] = true;
	}
	...
	/**
	 * Check if this notice is already printed.
	 */
	private function is_already_printed() : bool {
		// And id is required to check if already printed, so create a unique id based on the message.
		if ( ! isset( $this->id ) ) {
			$this->id = wp_json_encode( $this->messages );
		}

		return isset( self::$admin_notices_printed[ $this->id ] );
	}
