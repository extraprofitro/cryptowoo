...
class CW_AdminMain {
	...
	public function cw_delete_address_list_callback() {
		...
		if ( current_user_can( 'manage_options' ) && isset( $_POST['currency'] ) ) {
			...
			if ( cw_is_edit_wallet_disabled() ) {
				self::cryptowoo_log_data(
					0,
					__FUNCTION__,
					"$currency address list cannot be deleted because CRYPTOCURRENCY_PAYMENT_GATEWAY_DISABLE_EDIT_WALLET is enabled",
					'error'
				);

				echo wp_json_encode( __( 'Wallet editing is disabled. Changes to this field are not allowed.', 'cryptowoo' ) );
				wp_die();
			}
	...
	public function cw_handle_updates() {
		...
		if ( ! $version || version_compare( $version, CWOO_VERSION, '<' ) ) {
			...
			// Change autoload to false for existing address list to avoid concurrency issues when creating addresses.
			if ( $version && version_compare( $version, '1.6.4', '<' ) ) {
				foreach ( cw_get_cryptocurrencies() as $currency_code => $currency_name ) {
					$address_list = CW_AddressList::get_address_list( $currency_code );
					if ( ! empty( $address_list ) ) {
						// Autoload can only be changed when the option value changes.
						CW_AddressList::save_address_list( $currency_code, array() );
						CW_AddressList::save_address_list( $currency_code, $address_list );
					}
				}
			}
