...
class CW_Versions {
	...
	public static function check_addon_version_compatibility() {
		...
		if ( $hd_wallet_addon_version && version_compare( $hd_wallet_addon_version, '2.1.3', '<' ) ) {
		...
		if ( $dash_addon_version && version_compare( $dash_addon_version, '2.1.1', '<' ) ) {
		...
		if ( $vtc_addon_version && version_compare( $vtc_addon_version, '2.1.1', '<' ) ) {
		...
		if ( $eth_addon_version && version_compare( $eth_addon_version, '2.3.2', '<' ) ) {
		...
		if ( $xmr_addon_version && version_compare( $xmr_addon_version, '2.2.1', '<' ) ) {
		...
		if ( $dokan_addon_version && version_compare( $dokan_addon_version, '2.1.1', '<' ) ) {
		...
		if ( $solana_addon_version && version_compare( $solana_addon_version, '2.1.2', '<' ) ) {
	 ...
	 * This will add a button to update to the latest release if the update is available in wp-admin.
	 * If the update is not available, then $custom_link will be added as a button if it is a non-empty string.
	 *
	 * @param string $plugin_name The plugin name.
	 * @param string $custom_link Custom link to the plugin if an update is not available in wp-admin.
	 */
	public static function render_plugin_outdated_admin_notice( $plugin_name, $custom_link = 'https://www.cryptowoo.com/my-account/api-downloads/' ) {
		...
		$notice = CW_Admin_Notice::generate( CW_Admin_Notice::NOTICE_ERROR )
			...
			->add_message( sprintf( esc_html__( 'If you do not update now, you may be missing out on important security enhancements and the add-on may not function properly.', 'cryptowoo' ), $plugin_name, '<br>' ) )
			->add_message( sprintf( esc_html__( 'This may include exchange rate updates, order processing, settings, and more.', 'cryptowoo' ), $plugin_name, '<br>' ) )
		...
		if ( current_user_can( 'update_plugins' ) && isset( get_plugin_updates()[ $plugin_file ]->update ) ) {
			...
			$notice->add_button_with_full_path_url( esc_html__( 'Download Latest Release', 'cryptowoo' ), '', $custom_link );
		}
	 ...
	 * @return string
	 */
	public static function get_addon_version( string $version_constant, string $file ) : string {
